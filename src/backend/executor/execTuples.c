...
typedef void (*omrjit_fetch)(int32_t natts, TupleTableSlot *slot, HeapTuple tuple, uint32 *offp);
omrjit_fetch omr_fetch_attributes_init;
char		omrjit_fetch_path[MAXPGPATH];
 ...
 */
//New .so file
typedef void (*func_ptr)(char* a, char* b);
func_ptr init;
char		path_test[MAXPGPATH];
					   ...
					   int natts)
{
	...
	*offp = off;
	...
	//snprintf(path_test, MAXPGPATH, "%s/%s%s", "/usr/local/pgsql/lib", "jit_char_count_c", ".so");

	//init = (func_ptr)load_external_function(path_test, "jit_char_count_c", true, NULL);
	//init("aaaabbbbbbbcc","a");
}
